        ...
        <%
            ...
            if (Dept_name != null) {
                try {
                    ...
                    ResultSet rs = st.executeQuery("SELECT MAX(Dept_id) as id FROM department");
                    int id = 0;
                    if (rs != null) {
                        while (rs.next()) {
                            id = rs.getInt("id");
                    ...
                    id = id + 1;
                    int i = st.executeUpdate("insert into department(Dept_id,Dept_name)values('" + id + "','" + Dept_name + "')");
                    if (conn != null && !conn.isClosed()) {
                        conn.close();
                        Thread.sleep(1000);
                    }
                    response.sendRedirect("succInDep.jsp");
                } catch (Exception e) {
                    System.out.print(e);
                    e.printStackTrace();
                }
