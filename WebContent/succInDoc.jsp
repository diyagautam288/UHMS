        ...
        <%
            ...
            if (email != null) {
                try {
                    ...
                    if (c != 0) {
                        int i = st.executeUpdate("insert into reg_user(EMAIL,PASSWORD,F_NAME,L_NAME,GENDER,PHONE_NO,USER_TYPE,SECRET)values('" + email + "','" + password + "','" + first_name + "','" + last_name + "','" + Gender + "','" + phone_number + "','" + user_type + "','" + secret + "')");
                        out.println("Data is successfully inserted!");
                        rs = st.executeQuery("SELECT MAX(DOC_ID) as id FROM DOCTOR");
                        int id = 0;
                        if (rs != null) {
                            while (rs.next()) {
                                id = rs.getInt("id");
                        ...
                        id = id + 1;
                        int j = st.executeUpdate("insert into DOCTOR(DOC_ID,EMAIL,FEES,DEPT_ID)values('" + id + "','" + email + "','" + fees + "','" + dept_id + "')");
                ...
                response.sendRedirect("succInDoc.jsp");

            }
        %>

        <h1 style="text-align: center">SUCCESSFULLY ADDED!!</h1>
